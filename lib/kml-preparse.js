...
function splitFolders(kmlobj, callback){
		return kmlobj.kml.Document.forEach(function(DOC){
 			DOC.Folder.forEach(function(folder){
				var name = folder.name[0];
				console.dir(name);
				var doc =  JSON.parse(JSON.stringify(DOC));
				doc.Folder = [folder];			
				var kml = builder.buildObject(doc);
	      console.log('GroundOverlay included?', /GroundOverlay/i.test(kml))
	      // console.dir(callback)
	      if(callback && callback instanceof Function){
	      	var opts = {
	      		// format the filename
	      		// http://tinyurl.com/ow4bkxm
	      		filename : name.replace(/(-|\s|\"|\'|\\|\/|\||\(|\)|\+|=|\[|\]|{|}|&|\*|,|;|:|\>|\<|@|#|\$|%|!|`|~)/gi, '_') + '.json'
	      		, makeContainingFolder :  true
	      		, lowerCase : true
	      	}
	      	return callback(kml, opts)
	      	// return callback(kml, { filenameAddition : '_' + name  })
	      }
			})
		})
}
  ...
  parser.parseString(data, function (err, result) {
  	if(options.visible)
 		...
 		if(options.splitfolders)
 			return splitFolders(result, callback)
